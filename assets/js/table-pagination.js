(function() {
    document.addEventListener("DOMContentLoaded", function() {
        ...
        var customLists = document.querySelectorAll(".notif-list, .opportunity-list, .notification-list, .repayment-list, .funding-list");
        customLists.forEach(function(list) {
            var items = Array.from(list.children).filter(function(child) {
                return !child.classList.contains("no-records") && !child.classList.contains("no-search-results");
            });
    ...
    function isEmptyStateRow(row) {
        return row.classList.contains("no-records") ||
    ...
    function initPagination(container, items, type) {
        ...
        items.forEach(function(item) {
            var statusEl = item.querySelector(".badge, .wr-badge, .status-pill, .status, [class*='status-badge']");
            if (statusEl) {
        ...
        // Insert controls before the container/table
        ...
        // Insert pagination bar after the container/table
        container.parentNode.insertBefore(paginationBar, container.nextSibling);
        ...
        function applyFilters() {
            ...
            filteredItems = items.filter(function(item) {
                ...
                if (selectedStatus) {
                    var statusEl = item.querySelector(".badge, .wr-badge, .status-pill, .status, [class*='status-badge']");
                    var itemStatus = statusEl ? statusEl.textContent.trim().toUpperCase() : "";
        ...
        function render() {
            ...
            if (type === "table") {
                ...
                if (totalItems === 0) {
                    if (!noRecordsTr) {
                        ...
                        var tdCount = container.querySelector("thead tr") ? container.querySelectorAll("thead tr th").length : 10;
                        noRecordsTr.innerHTML = '<td colspan="' + tdCount + '" style="text-align: center; padding: 32px; color: #64748b;">No matching records found.</td>';
                ...
                if (totalItems === 0) {
                    if (!noRecordsDiv) {
                        ...
                        noRecordsDiv.style.cssText = "text-align: center; padding: 32px; color: #64748b; width: 100%; border: 1px dashed #cbd5e1; border-radius: 12px;";
            ...
            pages.forEach(function(page) {
                if (page === '...') {
                    ...
                    dots.textContent = "...";
                    dots.style.cssText = "color: #94a3b8; padding: 0 4px; font-weight: 600;";
        ...
        function stylePaginationButton(btn, isDisabled, isActive) {
            btn.style.cssText = "padding: 6px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 13.5px; font-weight: 600; cursor: pointer; transition: all 0.15s ease; outline: none;";
            if (isDisabled) {
